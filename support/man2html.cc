...
{
  char *newstr
      = (char *)realloc (old, (strlen (old) + len + 1) * sizeof (char));
...
static char escapesym = '\\', nobreaksym = '\'', controlsym = '.',
            fieldsym = 0, padsym = 0;
...
static const char *switchfont[16]
    = { "",      FC0 FO1, FC0 FO2, FC0 FO3, FC1 FO0, "",      FC1 FO2, FC1 FO3,
        FC2 FO0, FC2 FO1, "",      FC2 FO3, FC3 FO0, FC3 FO1, FC3 FO2, "" };
      ...
      while (c < h)
        {
          while (isspace (*c))
            c++;
          for (i = 0; tableopt[i] && strncmp (tableopt[i], c, tableoptl[i]);
               i++)
            ;
          c = c + tableoptl[i];
          switch (i)
          ...
          if (c[-1] == '\n' && c[1] == '\n')
            {
              if (currow->prev)
                ...
                {
                  currow->prev = layout
                      = (TABLEROW *)xmalloc (sizeof (TABLEROW));
  ...
  else
    {
      ...
      switch (i)
        ...
        case V ('S', 'H'):
          ...
          c = mandoc_command ? scan_troff_mandoc (c, 1, NULL)
                             : scan_troff (c, 1, NULL);
          if (mode)
            out_html ("</H4>\n");
          else
              ...
              if (words > 1)
                {
                  ...
                  *sl = '\0';
                  ...
                  sprintf (th_page_and_sec, "%s(%s)", wordlist[0],
                           wordlist[1]);
                  if (words > 2)
                    {
          ...
          if (owndef)
            {
              ...
              *sl = '\0';
              ...
              for (i = 0;
                   (((char *)owndef->st)[deflen + 2 + i] = owndef->st[i]); i++)
                ;
              ...
              if (mandoc_command)
                {
                  scan_troff_mandoc ((char *)(owndef->st + deflen + 2), 0,
                                     NULL);
                }
              else
                {
  ...
  while (*h && (!san || newline_for_fun || *h != '\n'))
    {
      ...
      else if (mandoc_line && *(h) && isupper (*(h)) && *(h + 1)
               && islower (*(h + 1)) && *(h + 2) && isspace (*(h + 2)))
        {
          /*
...
scan_troff_mandoc (char *c, int san, char **result)
{
  ...
  if (end > c + 2 && ispunct (*(end - 1)) && isspace (*(end - 2))
      && *(end - 2) != '\n')
    {
      /*
