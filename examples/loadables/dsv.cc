...
// Loadable class for "dsv".
class ShellLoadable : public Shell
{
public:
  int dsv_builtin (WORD_LIST *);
          ...
          for (field = ++prev; *field; field++)
            {
              if (qstate == DQUOTE && *field == '"' && field[1] == '"'
                  && (flags & F_SHELLQUOTE) == 0)
                buf[b++] = *field++; /* skip double quote */
              else if (qstate == DQUOTE && (flags & F_SHELLQUOTE)
                       && *field == '\\'
...
int
ShellLoadable::dsv_builtin (WORD_LIST *list)
{
  int opt, rval, flags;
...
int
dsv_builtin_load (const char *name)
{
  return 1;
}
...
void
dsv_builtin_unload (const char *name)
{
}

const char *const dsv_doc[] = {
...
Shell::builtin dsv_struct (
    static_cast<Shell::sh_builtin_func_t> (&ShellLoadable::dsv_builtin),
    dsv_doc, "dsv [-a ARRAYNAME] [-d DELIMS] [-Sgp] string", nullptr,
    BUILTIN_ENABLED);
