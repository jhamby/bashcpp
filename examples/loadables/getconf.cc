...
// Loadable class for "getconf".
class ShellLoadable : public Shell
{
public:
  int getconf_builtin (WORD_LIST *);
  ...
  switch (c->call)
    ...
    case PATHCONF:
      ...
      if (value == -1L)
        {
          if (errno)
            builtin_error ("pathconf %s: %s: %s", c->name, vpath,
                           strerror (errno));
          printf ("undefined\n");
          return EXECUTION_FAILURE;
        }
...
int
ShellLoadable::getconf_builtin (WORD_LIST *list)
{
  const struct conf *c;
...
static const char *const getconf_doc[]
    = { "Display values of system limits and options.", "",
        "getconf writes the current value of a configurable system limit or",
        "option variable to the standard output.", nullptr };

Shell::builtin getconf_struct (
    static_cast<Shell::sh_builtin_func_t> (&ShellLoadable::getconf_builtin),
    getconf_doc,
    "getconf -[ah] or getconf [-v spec] sysvar or getconf [-v spec] pathvar "
    "pathname",
