...
// $SHORT_DOC declare [-aAfFgiIlnrtux] [name[=value] ...] or declare -p [-aAfFilnrtux] [name ...] Set variable values and attributes.
...
// $BUILTIN typeset
// $FUNCTION declare_builtin
// $SHORT_DOC typeset [-aAfFgiIlnrtux] name[=value] ... or typeset -p [-aAfFilnrtux] [name ...] Set variable values and attributes.
...
int
Shell::declare_builtin (WORD_LIST *list)
{
...
int
Shell::local_builtin (WORD_LIST *list)
{
...
static char *
declare_build_newname (const std::string &name,
                       const std::string &subscript_start, size_t offset,
                       const std::string &value, int aflags)
{
...
static char *
declare_transform_name (const std::string &name, int flags_on, int flags_off)
{
  SHELL_VAR *var, *v;
  char *newname;
  ...
  if (list == 0) /* declare -[aAfFilnrtux] */
    {
      ...
      if (local_var)
        ...
        return set_builtin (nullptr);
      else
        set_or_show_attributes (nullptr, flags_on, nodefs);
  ...
  if (pflag) /* declare -p [-aAfFilnrtux] [name ...] */
    {
      ...
      return (
          sh_chkwrite (any_failed ? EXECUTION_FAILURE : EXECUTION_SUCCESS));
    }

  /* Some option combinations that don't make any sense */
  if ((flags_on & att_function)
      && (flags_on & (att_array | att_assoc | att_integer | att_nameref)))
    {
      char *optchar;
...
#define NEXT_VARIABLE()                                                       \
  ...
  while (list) /* declare [-aAfFilnrtux] name[=value] [name[=value] ...] */
...
#endif /* DEBUGGER */
                    {
                      t = nodefs ? name_cell (var)
                                 : named_function_string (
                                     name, function_cell (var),
      ...
      if (flags_on & att_nameref)
        {
          ...
          if (valid_array_reference (name, 0))
            {
              builtin_error (_ ("%s: reference variable cannot be an array"),
                             name);
              any_failed++;
              NEXT_VARIABLE ();
            }
          ...
          if (value && *value && (aflags & ASS_APPEND) == 0
            ...
            {
              builtin_error (
                  _ ("`%s': invalid variable name for name reference"), value);
              assign_error++;
              NEXT_VARIABLE ();
            }
        }
    ...
    restart_new_var_name:
      ...
      /* The rest of the loop body deals with declare -[aAlinrtux] name
         [name...] where each NAME can be an assignment statement. */

      subscript_start = nullptr; /* used below */
#if defined(ARRAY_VARS)
      ...
      if (variable_context && mkglobal == 0)
        {
          ...
          if (flags_on & att_assoc)
            var = make_local_assoc_variable (newname,
                                             MKLOC_ARRAYOK | inherit_flag);
          else if ((flags_on & att_array) || making_array_special)
            var = make_local_array_variable (newname,
                                             MKLOC_ASSOCOK | inherit_flag);
          else
#endif
              if (offset == 0 && (flags_on & att_nameref))
            {
              ...
              /* VARIABLE_CONTEXT != 0, so we are attempting to create or
                 modify the attributes for a local variable at the same scope.
                 If we've used a reference from a previous context to resolve
                 VAR, we want to throw REFVAR and VAR away and create a new
                 local var. */
              ...
              else if ((var = find_variable (name)) == 0
            ...
            var = make_local_variable (
                (flags_on & att_nameref) ? name : newname,
                inherit_flag); /* sets att_invisible for new vars */
          ...
          if (var && nameref_p (var) && readonly_p (var) && nameref_cell (var)
      ...
      if (var == 0 && (flags_on & att_nameref))
        {
          ...
          var = mkglobal ? find_global_variable_noref (name)
      ...
      /* However, if we're turning off the nameref attribute on an existing
      ...
      else if (var == 0 && (flags_off & att_nameref))
        {
          ...
          refvar = mkglobal ? find_global_variable_last_nameref (name, 0)
                            : find_variable_last_nameref (name, 0);
      ...
      */
          ...
          if (refvar && var == 0)
            {
              ...
              newname = declare_build_newname (nameref_cell (refvar),
                                               subscript_start, offset, value,
                                               aflags);
              free (name);
              name = newname;
      ...
      if (var == 0)
        ...
        /* At this point, VAR is the variable we are dealing with; REFVAR is
           the nameref variable we dereferenced to get VAR, if any. */
#if defined(ARRAY_VARS)
      var_exists = var != 0;
...
#endif
            var = mkglobal
                      ? bind_global_variable (name, nullptr, ASS_FORCE)
                      : bind_variable (name, nullptr, ASS_FORCE);
      ...
      /* Can't have an invalid identifier as nameref value */
               ...
               && valid_nameref_value (value, 1) == 0)
        {
          builtin_error (_ ("`%s': invalid variable name for name reference"),
                         value);
          any_failed++;
          NEXT_VARIABLE ();
        }
      ...
      else if ((flags_on & att_nameref) && nameref_p (var) == 0
               && var_isset (var) && offset == 0
      ...
      /* Cannot use declare +a name or declare +A name to remove an array
       * variable. */
      if (((flags_off & att_array) && array_p (var))
          || ((flags_off & att_assoc) && assoc_p (var)))
        {
          builtin_error (_ ("%s: cannot destroy array variables in this way"),
                         name);
          any_failed++;
          NEXT_VARIABLE ();
        }
        ...
        {
          builtin_error (_ ("%s: cannot convert indexed to associative array"),
                         name);
          any_failed++;
          NEXT_VARIABLE ();
        }
      ...
      else if ((making_array_special || creating_array || array_exists)
               && offset)
        {
          int vlen;
          vlen = STRLEN (value);
          /*itrace("declare_builtin: name = %s value = %s flags = %d", name,
           * value, wflags);*/
      ...
      /* I don't believe this condition ever tests true, but array variables
         may not be namerefs */
      if (array_p (var) || assoc_p (var) || compound_array_assign
          || simple_array_assign)
        onref = 0;
#endif
      ...
      if (offset && compound_array_assign)
      ...
      else if (simple_array_assign && subscript_start)
        {
          ...
          *subscript_start = '['; /* ] */
          ...
          var = assign_array_element (
              name, value, local_aflags,
              (array_eltstate_t *)0); /* XXX - not aflags */
          *subscript_start = '\0';
            ...
            if (v == 0 && (onref || nameref_p (var)))
              {
                ...
                if (created_var)
                  delete_var (name_cell (var),
                              mkglobal ? global_variables : shell_variables);
      ...
      if ((flags_on & (att_exported | att_readonly)) && tempvar_p (var))
        {
          ...
          if (tv)
            {
              tvalue = var_isset (var) ? savestring (value_cell (var))
                                       : savestring ("");
