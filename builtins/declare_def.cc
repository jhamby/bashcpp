...
#define NEXT_VARIABLE()                                                       \
  ...
  while (list) /* declare [-aAfFirx] name [name ...] */
    ...
    restart_new_var_name:
      ...
      if ((posixly_correct || (flags_on & att_function) == 0)
          ...
          && ((flags_on & att_function) == 0))
        {
          ...
          if (var == 0)
                   ...
                   && (flags_off & att_nameref) == 0)
            {
              ...
              newname = (refvar && refvar->context != variable_context)
                            ? name
                            : var->name;
              refvar = (SHELL_VAR *)NULL;
            }
	  ...
	  /* Here's what ksh93 seems to do.  If we are modifying an existing
	     ...
	     we're assigning right now. */
          ...
          /* However, if we're turning off the nameref attribute on an existing
          ...
          else if (var == 0 && (flags_off & att_nameref))
            {
              ...
              if (refvar)
                ...
                var = mkglobal ? find_global_variable (nameref_cell (refvar))
...
#if defined(ARRAY_VARS)
                  if (subscript_start)
                    strcpy (name + strlen (nameref_cell (refvar)),
                            subscript_start);
#endif
          ...
          if (var == 0)
            {
              ...
              if (flags_on & att_assoc)
                ...
                {
                  var = mkglobal
                            ? bind_global_variable (name, (char *)NULL,
                                                    ASS_FORCE)
                            : bind_variable (name, (char *)NULL, ASS_FORCE);
                  if (var && offset == 0)
                    VSETATTR (var, att_invisible);
                }
              if (var == 0)
          ...
          /* Can't take an existing array variable and make it a nameref */
          else if ((array_p (var) || assoc_p (var))
          ...
          else if (flags_on & att_nameref)
            {
              ...
              if (nameref_p (var) == 0 && var_isset (var) && offset == 0
                  && valid_nameref_value (value_cell (var), 0) == 0)
                {
                  builtin_error (
                      _ ("`%s': invalid variable name for name reference"),
                      value_cell (var));
                  any_failed = true;
                  NEXT_VARIABLE ();
                }
              ...
              VUNSETATTR (var, att_integer | att_uppercase | att_lowercase
                                   | att_capcase);
            }
          ...
          if (array_subscript_assignment && array_exists
              && creating_array == 0)
                   ...
                   && offset)
            {
              int vlen;
              vlen = STRLEN (value);
              /*itrace("declare_builtin: name = %s value = %s flags = %d",
               * name, value, wflags);*/
              if (shell_compatibility_level > 43
                  && (wflags & W_COMPASSIGN) == 0 && value[0] == '('
                  && value[vlen - 1] == ')')
                {
          ...
          if (((flags_off & att_array) && array_p (var))
            ...
            {
              builtin_error (
                  _ ("%s: cannot destroy array variables in this way"), name);
              any_failed = true;
              NEXT_VARIABLE ();
            }
          ...
          if ((flags_on & att_array) && assoc_p (var))
            {
              builtin_error (
                  _ ("%s: cannot convert associative to indexed array"), name);
              any_failed = true;
              NEXT_VARIABLE ();
            }
          if ((flags_on & att_assoc) && array_p (var))
            {
              builtin_error (
                  _ ("%s: cannot convert indexed to associative array"), name);
              any_failed = true;
              NEXT_VARIABLE ();
            }
          ...
          if (array_p (var) || assoc_p (var)
              || (offset && compound_array_assign) || simple_array_assign)
                ...
                if (v == 0 && (onref || nameref_p (var)))
                  {
                    ...
                    if (created_var)
                      delete_var (var->name, mkglobal ? global_variables
                                                      : shell_variables);
  ...
  return (assign_error
              ? EX_BADASSIGN
              : ((!any_failed) ? EXECUTION_SUCCESS : EXECUTION_FAILURE));
}
