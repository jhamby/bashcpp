  ...
  switch (ri)
    ...
    case r_move_output:
      if ((flags & RX_ACTIVE) && (redirect->rflags & REDIR_VARASSIGN))
        {
          redirector = fcntl (redir_fd, F_DUPFD,
                              SHELL_FD_BASE); /* XXX try this for now */
          r = errno;
          if (redirector < 0)
            sys_error (_ ("redirection error: cannot duplicate fd"));
      ...
      if ((flags & RX_ACTIVE) && (redir_fd != redirector))
        {
          ...
          if ((flags & RX_UNDOABLE)
              && (ri == r_move_input || ri == r_move_output))
            {
	  ...
	  if (((fcntl (redir_fd, F_GETFD, 0) == 1) || redir_fd < 2 || (flags & RX_CLEXEC)) &&
	       (redirector > 2))
#else
          if (((fcntl (redir_fd, F_GETFD, 0) == 1)
               || (redir_fd < 2 && (flags & RX_INTERNAL))
               || (flags & RX_CLEXEC))
              && (redirector > 2))
#endif
          SET_CLOSE_ON_EXEC (redirector);
          ...
          if ((redirect->flags & RX_INTERNAL)
              && (redirect->flags & RX_SAVCLEXEC) && redirector >= 3
              && (redir_fd >= SHELL_FD_BASE || (redirect->flags & RX_SAVEFD)))
...
add_undo_redirect (int fd, enum r_instruction ri, int fdbase)
{
  ...
  new_fd = fcntl (fd, F_DUPFD,
                  (fdbase < SHELL_FD_BASE) ? SHELL_FD_BASE : fdbase + 1);
  if (new_fd < 0)
