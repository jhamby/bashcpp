...
/* The maximum and minimum values for the integer type T.  These
   ...
   your host.  */
#define TYPE_MINIMUM(t) ((t)(!TYPE_SIGNED (t) ? (t)0 : ~TYPE_MAXIMUM (t)))
               ...
               const struct tm *tp)
{
  ...
  return (60 * (60 * (24 * days + (hour - tp->tm_hour)) + (min - tp->tm_min))
          + (sec - tp->tm_sec));
}
...
print_tm (struct tm *tp)
{
  printf ("%04d-%02d-%02d %02d:%02d:%02d yday %03d wday %d isdst %d",
          tp->tm_year + TM_YEAR_BASE, tp->tm_mon + 1, tp->tm_mday, tp->tm_hour,
          tp->tm_min, tp->tm_sec, tp->tm_yday, tp->tm_wday, tp->tm_isdst);
}

static int
check_result (time_t tk, struct tm tmk, time_t tl, struct tm tml)
{
...
main (int argc, char **argv)
{
  ...
  if ((argc == 3 || argc == 4)
      && (sscanf (argv[1], "%d-%d-%d%c", &tm.tm_year, &tm.tm_mon, &tm.tm_mday,
                  &trailer)
          == 3)
      && (sscanf (argv[2], "%d:%d:%d%c", &tm.tm_hour, &tm.tm_min, &tm.tm_sec,
                  &trailer)
          == 3))
    {
