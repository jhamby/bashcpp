/* strvis.cc - make unsafe graphical characters in a string visible. */
...
int
Shell::sh_charvis (const char *s, size_t *sindp, size_t slen, char *ret,
                   size_t *rindp)
{
  unsigned char c;
  size_t si, ri;
...
/* Return a new string with `unsafe' non-graphical characters in S rendered
   in a visible way. */
char *
Shell::sh_strvis (const char *string)
{
  size_t slen, sind;
  char *ret;
