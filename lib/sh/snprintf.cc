...
/*
 build a test version with
   gcc -g -DDRIVER -I../.. -I../../include -o test-snprintf snprintf.c
 fmtu*long.o
*/
...
#define GETUNSIGNED(p)                                                        \
  (((p)->flags & PF_LONGINT)                                                  \
       ...
       : (((p)->flags & PF_SHORTINT)                                          \
              ? (unsigned long)(unsigned short)GETARG (int)                   \
...
#ifdef HAVE_LONG_DOUBLE
static void ldfallback (struct DATA *, const char *, const char *,
                        long double);
#endif
...
#define GETLOCALEDATA(d, t, g)                                                \
  ...
  while (0);
#else
#define GETLOCALEDATA(d, t, g) ((d) = '.', (t) = ',', g = "\003")
#endif
...
static int
chkinfnan (struct DATA *p, double d, int mode) /* == 1 for inf, == 2 for nan */
{
  int i;
  char *tmp;
  char *big, *small;
...
#endif
             ((p->flags & PF_SPACE) ? 1 : 0) - strlen (tmp) - p->precision
  ...
  while (se > s)
    {
      *--re = *--se;
      ...
      if (--len == 0 && se > s)
        {
          *--re = thoussep;
          len = *++g; /* was g++, but that uses first char twice (glibc bug,
                         too) */
          if (*g == '\0')
...
static void
init_data (struct DATA *p, char *string, size_t length, const char *format,
           int mode)
{
...
static int
vsnprintf_internal (struct DATA *data, char *string, size_t length,
      ...
      for (state = 1; state && *data->pf;)
        {
          ...
          switch (c)
            ...
            case 'h':
              data->flags
                  |= (data->flags & PF_SHORTINT) ? PF_SIGNEDCHAR : PF_SHORTINT;
              continue;
            case 'l':
              data->flags
                  |= (data->flags & PF_LONGINT) ? PF_LONGLONG : PF_LONGINT;
              continue;
            case 'L':
