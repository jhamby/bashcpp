...
unsigned int
Readline::noninc_search_from_pos (const char *string, unsigned int pos,
                                  int dir, rl_search_flags flags,
                                  unsigned int *ncp)
{
  unsigned int ret;
  const char *s;
...
Readline::noninc_dosearch (const char *string, int dir, rl_search_flags flags)
{
  ...
  unsigned int ind;
  unsigned int pos = noninc_search_from_pos (
      string,
      static_cast<unsigned int> (static_cast<int> (noninc_history_pos) + dir),
      dir, flags, &ind);
  ...
  if (_rl_enable_active_region && ((flags & SF_PATTERN) == 0) && ind > 0
      && ind < rl_end ())
    {
      rl_point = ind;
...
rl_history_search_internal (int count, int dir)
{
  ...
  while (count)
	...
	break;
      ...
      prev_line_found
          = temp->line.c_str (); // XXX - could the ptr be freed too soon?
      count--;
    }
