/* bind.cc -- key binding and startup file support for the readline library. */
  ...
  if (!keyseq.empty ())
    {
      ...
      rl_command_func_t func = rl_function_of_keyseq_len (keys, kmap, nullptr);
       ...
       (it != seq.end () && (c = *it)) || has_control || has_meta; ++it)
    {
      ...
      */
      if (c == '\\' && (it + 1) != seq.end ())
        {
          ...
          switch (c)
            ...
            case 'x':
              ++it;
              for (temp = 2, c = 0;
                   it != seq.end () && ISXDIGIT (*it) && temp--; ++it)
                c = (c * 16) + HEXVALUE (*it);
      ...
      if (META_CHAR (c) && _rl_convert_meta_chars_to_ascii)
        {
          array.push_back (ESC); /* ESC is meta-prefix */
          array.push_back (static_cast<char> (UNMETA (c)));
        }
      else
        array.push_back (static_cast<char> (c));
...
Readline::rl_untranslate_keyseq (int seq)
{
  ...
  kseq.push_back (static_cast<char> (c));
  return kseq;
}
  ...
  for (it = keyseq.begin (); it != keyseq.end (); ++it)
    {
      ...
      else if (map[ic].type != ISKMAP && (it + 1) < keyseq.end ())
        return nullptr;
      else
        {
          if (type)
...
Readline::parser_if (char *args)
{
  ...
  if (rl_terminal_name && _rl_strnicmp (args, "term=", 5) == 0)
  ...
  else if (_rl_strnicmp (args, "version", 7) == 0)
    {
      ...
      size_t previ = i;
      op_type_t op = parse_comparison_op (args, &i);
      if (op <= 0)
        {
          _rl_init_file_error ("comparison operator expected, found `%s'",
                               args + previ);
      ...
      if (args[i] == 0 || _rl_digit_p (args[i]) == 0)
        {
          _rl_init_file_error ("numeric argument expected, found `%s'",
                               args + i);
          return 0;
        }
  ...
  else if ((boolvar = find_boolean_var (args)) >= 0
...
Readline::parser_else (char *)
{
  ...
  for (std::vector<bool>::iterator it = if_stack.begin ();
       it != if_stack.end (); ++it)
    if (*it)
      return 0;
...
} parser_directives[] = { { "if", &Readline::parser_if },
...
Readline::rl_parse_and_bind (char *string)
{
  ...
  if (i == 0)
    {
      _rl_init_file_error ("`%s': invalid key binding: missing key sequence",
                           string);
      return 1;
    }
  ...
  if (_rl_stricmp (string, "set") == 0)
    {
      ...
      value = var;
      while (*value && whitespace (*value) == 0)
        value++;
      if (*value)
        *value++ = '\0';
  ...
  if (*funname == '\'' || *funname == '"')
    {
      ...
      if (string[i])
        ...
        {
          _rl_init_file_error ("`%s': missing closing quote for macro",
                               funname);
          return 1;
        }
    }
...
Readline::init_boolean_varlist ()
{
  ...
  // Note: keep this list sorted in case-insensitive order, for binary search
  // by name.
  bvl.push_back (bvt ("bind-tty-special-chars", &_rl_bind_stty_chars, 0));
  bvl.push_back (
      bvt ("blink-matching-paren", &rl_blink_matching_paren, V_SPECIAL));
  bvl.push_back (bvt ("byte-oriented", &rl_byte_oriented, 0));
#if defined(COLOR_SUPPORT)
  bvl.push_back (
      bvt ("colored-completion-prefix", &_rl_colored_completion_prefix, 0));
  ...
  bvl.push_back (
      bvt ("enable-bracketed-paste", &_rl_enable_bracketed_paste, V_SPECIAL));
  ...
  bvl.push_back (
      bvt ("history-preserve-point", &_rl_history_preserve_point, 0));
  bvl.push_back (
      bvt ("horizontal-scroll-mode", &_rl_horizontal_scroll_mode, 0));
  ...
  bvl.push_back (
      bvt ("mark-symlinked-directories", &_rl_complete_mark_symlink_dirs, 0));
  bvl.push_back (bvt ("match-hidden-files", &_rl_match_hidden_files, 0));
  bvl.push_back (bvt ("menu-complete-display-prefix",
                      &_rl_menu_complete_prefix_first, 0));
  ...
  bvl.push_back (
      bvt ("prefer-visible-bell", &_rl_prefer_visible_bell, V_SPECIAL));
  bvl.push_back (bvt ("print-completions-horizontally",
  ...
  bvl.push_back (
      bvt ("show-all-if-unmodified", &_rl_complete_show_unmodified, 0));
...
Readline::sv_dispprefix (const char *value)
{
  ...
  _rl_completion_prefix_display_length = static_cast<size_t> (nval);
  return 0;
}
...
Readline::sv_histsize (const char *value)
{
  ...
  stifle_history (static_cast<size_t> (nval));
  return 0;
}
...
Readline::sv_bell_style (const char *value)
{
  ...
  else if (_rl_stricmp (value, "audible") == 0
           || _rl_stricmp (value, "on") == 0)
    _rl_bell_preference = AUDIBLE_BELL;
...
Readline::sv_isrchterm (const char *value)
{
  ...
  _rl_isearch_terminators.clear ();
  _rl_isearch_terminators_custom = true;

  /* Isolate the value and translate it into a character string. */
  if (value[0] == '"' || value[0] == '\'')
    {
      char delim = value[0];
      for (begin = end = 1; value[end] && value[end] != delim; end++)
        ;
    }
  else
    {
      for (begin = end = 0; value[end] && whitespace (value[end]) == 0; end++)
        ;
    }

  std::string tmp = std::string (value + begin, end - begin);

  /* Translate the value into a character string. */
  rl_translate_keyseq (tmp, _rl_isearch_terminators);
...
{
  _rl_emacs_mode_str.clear ();
  if (value && *value)
    {
      _rl_emacs_mode_str_custom = true;
      rl_translate_keyseq (value, _rl_emacs_mode_str);
    }
  else if (value)
    _rl_emacs_mode_str_custom = true; // empty prompt
  else
    _rl_emacs_mode_str_custom = false; // default prompt
...
{
  _rl_vi_ins_mode_str.clear ();
  if (value && *value)
    {
      _rl_vi_ins_mode_str_custom = true;
      rl_translate_keyseq (value, _rl_vi_ins_mode_str);
    }
  else if (value)
    _rl_vi_ins_mode_str_custom = true; // empty prompt
  else
    _rl_vi_ins_mode_str_custom = false; // default prompt
...
{
  _rl_vi_cmd_mode_str.clear ();
  if (value && *value)
    {
      _rl_vi_cmd_mode_str_custom = true;
      rl_translate_keyseq (value, _rl_vi_cmd_mode_str);
    }
  else if (value)
    _rl_vi_cmd_mode_str_custom = true; // empty prompt
  else
    _rl_vi_cmd_mode_str_custom = false; // default prompt
...
static const assoc_list name_key_alist[] = {
  { "DEL", 0x7f },    { "ESC", '\033' },   { "Escape", '\033' },
  { "LFD", '\n' },    { "Newline", '\n' }, { "RET", '\r' },
  { "Return", '\r' }, { "Rubout", 0x7f },  { "Space", ' ' },
  { "SPC", ' ' },     { "Tab", 0x09 },
};
...
_rl_get_keyname (int key)
{
  ...
  if (c >= 128 && c <= 159)
    {
      ...
      keyname.push_back ('2');
      c -= 128;
      keyname.push_back (static_cast<char> (c / 8) + '0');
      c = (c % 8) + '0';
    }
  ...
  /* Now add the key, and return it. */
  keyname.push_back (static_cast<char> (c));
  return keyname;
}

// Return a vector of strings which represent the key sequences that are used
// to invoke FUNCTION in MAP.
      ...
      switch (map[key].type)
        ...
        case ISFUNC:
          ...
          if (map[key].value.function == function)
            {
              std::string keyname = _rl_get_keyname (key);
        ...
        case ISKMAP:
          {
            ...
            if (map[key].value.map)
              seqs = rl_invoking_keyseqs_in_map (function, map[key].value.map);
            else
              break;
  ...
  for (int i = 0; (name = names[i]); i++)
    {
      ...
      std::vector<std::string> invokers
...
Readline::_rl_get_string_variable_value (const char *name)
{
  if (_rl_stricmp (name, "bell-style") == 0)
    ...
    return !_rl_comment_begin.empty () ? _rl_comment_begin.c_str ()
    ...
    {
      if (!_rl_isearch_terminators_custom)
        return nullptr;
      std::string ret
...
{
  for (std::vector<bvt>::iterator it = boolean_varlist_.begin ();
