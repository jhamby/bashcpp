...
Readline::stat_char (const char *filename)
{
  ...
  if (S_ISDIR (finfo.st_mode))
  ...
  else if (S_ISREG (finfo.st_mode))
    {
      ...
      ext = strrchr (fn, '.');
      if (ext
          && (_rl_stricmp (ext, ".exe") == 0 || _rl_stricmp (ext, ".cmd") == 0
...
const char *
Readline::printable_part (const char *pathname)
{
  ...
  if (temp == nullptr && c_isalpha (pathname[0]) && pathname[1] == ':')
    temp = pathname + 1;
#endif

  if (temp == nullptr || *temp == 0 || (temp[1] == 0 && temp == pathname))
    return pathname;
...
size_t
Readline::fnprint (const char *to_print, size_t prefix_bytes,
                   const char *real_pathname)
{
  size_t print_len = strlen (to_print);
#if defined(HANDLE_MULTIBYTE)
  mbstate_t ps;
  size_t width;
  wchar_t wc;

  const char *end = to_print + print_len + 1;
  memset (&ps, 0, sizeof (mbstate_t));
#endif
  ...
  const char *s = to_print + prefix_bytes;
  while (*s)
    {
      if (CTRL_CHAR (*s))
        {
        ...
        {
#if defined(HANDLE_MULTIBYTE)
          size_t tlen = mbrtowc (&wc, s, static_cast<size_t> (end - s), &ps);
      ...
      if (common_prefix_len > 0
          && static_cast<size_t> (s - to_print) >= common_prefix_len)
        {
#if defined(COLOR_SUPPORT)
...
size_t
Readline::print_filename (char *to_print, char *full_pathname,
                          size_t prefix_bytes)
{
      ...
      if (to_print != full_pathname)
        {
          ...
          strcpy (new_full_pathname, s.data ());
          if (s[slen - 1] == '/')
            slen--;
          else
          ...
          if (rl_visible_stats)
            ...
            {
              if (rl_directory_completion_hook == nullptr
...
std::string
Readline::rl_quote_filename (string_view s, replace_type, char *qcp)
{
  std::string ret;
...
Readline::_rl_find_completion_word (rl_qf_flags *fp, unsigned char *dp)
{
  ...
  const char *lbuffer_cstr = rl_line_buffer.c_str ();
  ...
  if (rl_completer_quote_characters)
    {
      ...
      for (scan = 0; scan < end;
           scan = MB_NEXTCHAR (lbuffer_cstr, scan, 1, MB_FIND_ANY))
        {
          if (pass_next)
            {
              pass_next = false;
          ...
          /* Shell-like semantics for single quotes -- don't allow backslash
             ...
             of quote_char. */
          if (quote_char != '\'' && lbuffer_cstr[scan] == '\\')
            {
              pass_next = true;
          ...
          if (quote_char != '\0')
            {
              ...
              if (lbuffer_cstr[scan] == quote_char)
                {
                           ...
                           lbuffer_cstr[scan]))
            {
              ...
              quote_char = static_cast<unsigned char> (lbuffer_cstr[scan]);
              rl_point = scan + 1;
  ...
  if (rl_point == end && quote_char == '\0')
    {
      ...
      while ((rl_point = MB_PREVCHAR (lbuffer_cstr, rl_point, MB_FIND_ANY)))
        {
          char c = lbuffer_cstr[rl_point];

          if (strchr (brkchars, c) == nullptr)
            continue;

          /* Call the application-specific function to tell us whether
             this word break character is quoted and should be skipped. */
          if (rl_char_is_quoted_p && found_quote
              && ((*this).*rl_char_is_quoted_p) (rl_line_buffer, rl_point))
            continue;
  ...
  if (c)
    {
      ...
      if (rl_char_is_quoted_p)
        isbrk = (found_quote == 0
                 || ((*this).*rl_char_is_quoted_p) (
                        rl_line_buffer, static_cast<int> (rl_point))
                        == 0)
                && strchr (brkchars, c) != nullptr;
      else
        isbrk = strchr (brkchars, c) != nullptr;
      ...
      if (isbrk)
        {
          ...
          if (rl_special_prefixes == nullptr
              || strchr (rl_special_prefixes, c) == nullptr)
            rl_point++;
        }
    }
...
/* Filter out duplicates in MATCHES.  This frees up the strings in
   MATCHES. */
void
Readline::remove_duplicate_matches (std::vector<char *> &matches)
{
  if (matches.size () < 2)
    return;
  ...
  for (it = matches.begin () + 1; it < matches.end (); ++it)
    {
      if (strcmp ((*(it - 1))->c_str (), (*it)->c_str ()) == 0)
        {
          delete[] * it;
          it = matches.erase (it);
        }
...
void
Readline::compute_lcd_of_matches (std::vector<char *> &match_list,
                                  string_view text)
{
      ...
      for (si = 0; (c1 = s1[si]) && (c2 = s2[si]); si++)
        {
          ...
          if (MB_CUR_MAX > 1 && !rl_byte_oriented)
            {
              v1 = mbrtowc (&wc1, s1 + si, strlen (s1 + si), &ps1);
              v2 = mbrtowc (&wc2, s2 + si, strlen (s2 + si), &ps2);
              ...
              if (_rl_completion_case_fold)
                {
                  wc1 = static_cast<wchar_t> (
                      towlower (static_cast<wint_t> (wc1)));
                  wc2 = static_cast<wchar_t> (
                      towlower (static_cast<wint_t> (wc2)));
                }
              if (wc1 != wc2)
                break;
              else if (v1 > 1)
      ...
      if (_rl_completion_case_fold)
        {
          /* We're making an assumption here:
                IF we're completing filenames AND
                   the application has defined a filename dequoting function
             AND we found a quote character AND the application has requested
          ...
          for (it = match_list.begin () + 1; it < match_list.end (); ++it)
            if (strncmp ((*it)->c_str (), text.c_str (), lx) == 0)
              {
...
int
Readline::postprocess_matches (std::vector<char *> &matches,
...
void
Readline::rl_display_match_list (std::vector<char *> &matches,
                                 size_t max)
{
  ...
  if (_rl_completion_prefix_display_length > 0)
    {
      ...
      sind = static_cast<int> (temp ? strlen (temp) : strlen (t));
      if (common_length > max || sind > max)
        common_length = sind = 0;

      if (common_length
              > static_cast<int> (_rl_completion_prefix_display_length)
...
void
Readline::display_matches (const std::vector<char *> &matches)
{
  int len, max, i;
  ...
  if (matches[1] == nullptr)
    {
      const char *temp = printable_part (matches[0]);
      rl_crlf ();
  ...
  if (should_quote)
    {
      ...
      should_quote = rl_filename_quote_characters
                         ? (_rl_strpbrk (match, rl_filename_quote_characters)
                            != nullptr)
                         : 0;
      ...
      if (do_replace != NO_MATCH && rl_filename_quoting_function)
        replacement
            = ((*this).*rl_filename_quoting_function) (match, do_replace, qc);
    }
  return replacement;
}
  ...
  if (replacement)
    {
      ...
      if (qc && *qc && end && rl_line_buffer[rl_point] == *qc
                           ...
                           unsigned char quote_char, bool nontrivial_match)
{
  ...
  if (delimiter)
    ...
    temp_string[temp_string_index++]
        = static_cast<char> (rl_completion_append_character);
...
void
Readline::insert_all_matches (const std::vector<char *> &matches,
...
Readline::rl_complete_internal (int what_to_do)
{
  ...
  char *saved_line_buffer
      = rl_line_buffer.empty () ? nullptr : savestring (rl_line_buffer);
  ...
  std::vector<char *> *matches = gen_completion_matches (
  ...
  switch (what_to_do)
    ...
    case '@':
      ...
      if (what_to_do == TAB)
        {
          if (*matches[0])
            insert_match (matches[0], start,
                          matches[1] ? MULT_MATCH : SINGLE_MATCH, &quote_char);
        }
  ...
  if (saved_line_buffer)
    {
      _rl_cmpl_changed_buffer
          = strcmp (rl_line_buffer.c_str (), saved_line_buffer) != 0;
...
/* Return a vector of string which is a list of completions for TEXT.
   ...
   ENTRY_FUNCTION is a function of two args, and returns a const char *.
 ...
 */
std::vector<char *> *
Readline::rl_completion_matches (string_view text,
                                 rl_compentry_func_t entry_function)
{
  /* The list of matches. */
  std::vector<char *> *match_list = new std::vector<char *> ();
...
/* A completion function for usernames.
   ...
   character (usually `~').  */
std::string *
Readline::rl_username_completion_function (string_view text, int state)
{
  ...
  while ((_rl_ucmp_pwentry = getpwent ()))
    {
      ...
      if (_rl_ucmp_username.empty ()
          || _rl_ucmp_username != _rl_ucmp_pwentry->pw_name)
        break;
    }
#endif
  ...
  if (_rl_ucmp_pwentry == nullptr)
    ...
    {
      char *value = new char[2 + strlen (_rl_ucmp_pwentry->pw_name)];

      *value = *text;

      strcpy (value + _rl_ucmp_first_char_loc, _rl_ucmp_pwentry->pw_name);
...
bool
Readline::complete_fncmp (const char *convfn, const char *filename)
{
#if defined(HANDLE_MULTIBYTE)
  size_t v1, v2;
  mbstate_t ps1, ps2;
  ...
  if (_rl_completion_case_fold && _rl_completion_case_map)
          ...
          do
            {
              ...
              if (d)
                return false;
              s1++;
              s2++; /* already checked convlen >= filename_len */
            }
          while (--len != 0);
        }
    ...
    {
      if ((convfn[0] == filename[0]) && (convlen >= filename_len)
...
std::string *
Readline::rl_filename_completion_function (string_view text, int state)
{
  ...
  if (state == 0)
    {
      ...
      const char *temp = strrchr (_rl_fcmp_dirname.c_str (), '/');

#if defined(__MSDOS__) || defined(_WIN32)
      /* special hack for //X/... */
      if (dirname[0] == '/' && dirname[1] == '/' && c_isalpha (dirname[2])
          && dirname[3] == '/')
      ...
      else if (c_isalpha (dirname[0]) && dirname[1] == ':')
        {
      ...
      else if (!tilde_dirname && rl_completion_found_quote
  ...
  if (entry == nullptr)
  ...
  else
    {
      ...
      if (!_rl_fcmp_dirname.empty ()
          && (_rl_fcmp_dirname[0] != '.' || _rl_fcmp_dirname[1]))
        {
          if (rl_complete_with_tilde_expansion
              && _rl_fcmp_users_dirname[0] == '~')
            {
              ...
              strcpy (temp, _rl_fcmp_dirname.data ());
          ...
          else
            {
              ...
              temp = new char[2 + dirlen + D_NAMLEN (entry)];
              strcpy (temp, _rl_fcmp_users_dirname.data ());
...
Readline::rl_old_menu_complete (int count, int invoking_key)
{
  ...
  if (rl_last_func != &Readline::rl_old_menu_complete)
    {
      ...
      _rl_omenu_orig_text
          = rl_copy_text (_rl_omenu_orig_start, _rl_omenu_orig_end);
      _rl_omenu_matches = gen_completion_matches (
          _rl_omenu_orig_text, _rl_omenu_orig_start, _rl_omenu_orig_end,
          our_func, found_quote, _rl_omenu_quote_char);

      /* If we are matching filenames, the attempted completion function will
         have set rl_filename_completion_desired to a non-zero value.  The
         basic rl_filename_completion_function does this. */
      ...
      for (_rl_omenu_match_list_size = 0;
           _rl_omenu_matches[_rl_omenu_match_list_size];
  ...
  if (_rl_omenu_match_list_index == 0 && _rl_omenu_match_list_size > 1)
  ...
  else
    {
      ...
      append_to_match (
          _rl_omenu_matches[_rl_omenu_match_list_index], _rl_omenu_delimiter,
          _rl_omenu_quote_char,
...
Readline::rl_menu_complete (int count, int)
{
  ...
  if ((rl_last_func != &Readline::rl_menu_complete
       && rl_last_func != &Readline::rl_backward_menu_complete)
      || _rl_menu_full_completion)
    {
      /* Clean up from previous call, if any. */
      ...
      _rl_menu_orig_text
          = rl_copy_text (_rl_menu_orig_start, _rl_menu_orig_end);
      _rl_menu_matches = gen_completion_matches (
          _rl_menu_orig_text, _rl_menu_orig_start, _rl_menu_orig_end, our_func,
      ...
      if ((*_rl_menu_matches)[0])
        {
          insert_match (_rl_menu_matches[0], _rl_menu_orig_start,
                        ...
                        &_rl_menu_quote_char);
          _rl_menu_orig_end
              = _rl_menu_orig_start + strlen (_rl_menu_matches[0]);
          _rl_cmpl_changed_buffer
