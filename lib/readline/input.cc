...
Readline::_rl_insert_typein (int c)
{
  ...
  while ((t = rl_get_char (&key)) && _rl_keymap[key].type == ISFUNC
  ...
  while (1)
    {
      ...
      if (result >= 0)
        result = static_cast<int> (
            read (fileno (stream), &c, sizeof (unsigned char)));
  ...
  while (mb_len < size)
    {
      ...
      mbchar_bytes_length
          = mbrtowc (&wc, mbchar, static_cast<size_t> (mb_len), &ps);
...
Readline::_rl_read_mbstring (int first, char *mb, int mlen)
{
  ...
  c = first;
  memset (mb, 0, static_cast<size_t> (mlen));
