...
static char *hist_error(const char *, unsigned int, unsigned int, hist_error_type);
...
#define RETURN_ENTRY(e, w) \
  ...
  if (_rl_digit_p (string[i]))
	...
	which = (which * 10) + static_cast<unsigned int> (_rl_digit_value (string[i]));
...
#define FAIL_SEARCH() \
  do { \
    history_offset = history_length (); delete[] temp; return nullptr; \
  } while (0)
	  ...
	  if (substring_okay)
	    {
	      ...
	      history_search_match = history_find_word (entry->line.c_str (),
...
static inline void
hist_string_extract_single_quoted (const char *string, unsigned int *sindex, int flags)
{
  unsigned int i;
...
static char *
hist_error(const char *s, unsigned int start, unsigned int current, hist_error_type errtype)
{
  char *temp;
  const char *emsg;
...
int
History::history_expand_internal (const char *string, unsigned int start, char qc,
	...
	case 's':
	  {
		...
		char *t = get_subst_pattern (string, &i, delimiter, 0, &history_subst_lhs_len);
		...
		if (t)
		...
		else if (!history_subst_lhs)
		  {
			...
			history_subst_lhs_len = static_cast<unsigned int> (std::strlen (history_subst_lhs));
		      }
		    else
		      {
		...
		delete[] history_subst_rhs;
		history_subst_rhs = get_subst_pattern (string, &i, delimiter,
						       1, &history_subst_rhs_len);
		...
		*ret_string = hist_error (string, starting_index, i, NO_PREV_SUBST);
	    ...
	    unsigned int l_temp = static_cast<unsigned int> (std::strlen (temp));
		...
		*ret_string = hist_error (string, starting_index, i, SUBST_FAILED);
		...
		if (STREQN (temp + si, history_subst_lhs, history_subst_lhs_len))
		  {
		    unsigned int len = history_subst_rhs_len - history_subst_lhs_len + l_temp;
		    new_event = new char[1 + len];
		    std::strncpy (new_event, temp, si);
		    std::strncpy (new_event + si, history_subst_rhs, history_subst_rhs_len);
			...
			l_temp = static_cast<unsigned int> (std::strlen (temp));
			substitute_globally++;
			continue;
		      }
...
History::history_expand (const char *hstring, char **output)
{
  ...
  for (unsigned int i = 0; i < l; i++)
	...
	case -3:		/* history_expansion_char */
	  ...
	  unsigned int eindex;
	  char *temp;
	  int r = history_expand_internal (string, i, qc, &eindex, &temp, result.c_str());
	  if (r < 0)
	    {
	      *output = temp;
	      if (string != hstring)
				     ...
				     unsigned int *caller_index)
{
  ...
  if (spec[i] == '%')
    {
      *caller_index = i + 1;
      return history_search_match ? savestring (history_search_match) : savestring ("");
    }
  ...
  for (unsigned int i = 0; string[i]; )
      ...
      if (i == start && history_word_delimiters)
	{
	 ...
	 particular index falls, remember it. */
      if (indp && wind != static_cast<unsigned int> (-1) && wind >= start && wind < i)
        *indp = static_cast<unsigned int> (result.size());	// this will be the index of the new entry
