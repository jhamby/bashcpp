...
#if 0
// FIXME: move to Readline class
extern void _rl_free_history_entry (HIST_ENTRY *);

#if defined (COLOR_SUPPORT)
extern void _rl_parse_colors ();		/* XXX */
#endif
...
static void bind_bracketed_paste_prefix ();

static void readline_default_bindings ();
static void reset_default_bindings ();

static int _rl_subseq_result (int, Keymap, int, int);
static int _rl_subseq_getchar (int);


#endif  // more stuff to move to readline.h
...
Readline::Readline (const char *search_delimiter_chars,
	...
	_paren_blink_usec (500000),
	rl_arg_sign (1),
	_rl_keyseq_timeout (500),
	_keyboard_input_timeout (100000),
	_rl_vi_last_command ('i'),
	_rl_vi_last_repeat (1),
	_rl_vi_last_arg_sign (1),
	_rl_yank_count_passed (1),
	...
	rl_sort_completion_matches (true),
	rl_completion_append_character (' '),
	_rl_term_autowrap (-1),
	_rl_prefer_visible_bell (true),
  ...
  if (entry && rl_undo_list)
    {
      ...
      rl_revert_line (1, 0);
      entry = replace_history_entry (where_history (), rl_line_buffer, nullptr);
      delete entry;
...
#if defined (READLINE_CALLBACKS)
int
Readline::_rl_dispatch_callback (_rl_keyseq_cxt *cxt)
{
  int nkey, r;
...
int
Readline::_rl_dispatch (int key, Keymap map)
{
...
int
Readline::_rl_dispatch_subseq (int key, Keymap map, int got_subseq)
{
  int r, newkey;
  char *macro;
...
#  endif
	  if (RL_ISSTATE (RL_STATE_CALLBACK))
		...
		cxt->flags |= KSEQ_SUBSEQ;
	      ...
	      cxt->subseq_arg = got_subseq || cxt->dmap[ANYOTHERKEY].value.function;
	  ...
	  r = _rl_dispatch_subseq (newkey, _rl_dispatching_keymap,
				   got_subseq || map[ANYOTHERKEY].value.function);
...
int
Readline::_rl_subseq_result (int r, Keymap map, int key, int got_subseq)
{
  if (r == -2)
...
void
Readline::readline_initialize_everything ()
{
...
void
Readline::readline_default_bindings ()
{
...
void
Readline::reset_default_bindings ()
{
  if (_rl_bind_stty_chars)
    {
...
void
Readline::bind_arrow_keys_internal (Keymap map)
{
  Keymap xkeymap;
...
void
Readline::bind_bracketed_paste_prefix ()
{
  Keymap xkeymap;
  ...
  _rl_keymap = emacs_standard_keymap ();
  rl_bind_keyseq_if_unbound (BRACK_PASTE_PREF, &Readline::rl_bracketed_paste_begin);
...
int
Readline::rl_save_state (struct readline_state *sp)
{
  if (sp == nullptr)
    return -1;
...
int
Readline::rl_restore_state (struct readline_state *sp)
{
  if (sp == nullptr)
    return -1;
