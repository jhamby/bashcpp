...
enum delete_job_flags
{
  DEL_WARNSTOPPED = 1, /* warn about deleting stopped jobs */
  DEL_NOBGPID = 2      /* don't add pgrp leader to bgpids */
};
...
void
Shell::save_pipeline (bool clear)
{
  sigset_t set, oset;
...
PROCESS *
Shell::restore_pipeline (bool discard)
{
...
int
Shell::stop_pipeline (bool async, COMMAND *deferred)
{
  JOB *newjob;
  sigset_t set, oset;
  ...
  if (interactive)
      ...
      for (i = js.j_lastj+1; i != js.j_lastj; i++)
	{
      ...
      for (i = js.j_lastj ? js.j_lastj + 1 : js.j_lastj; i < js.j_jobslots;
           i++)
        if (jobs[i] == 0)
          break;
#endif
  ...
  if ((!interactive_shell || subshell_environment) && i == js.j_jobslots
      && js.j_jobslots >= MAX_JOBS_IN_ARRAY)
    {
  ...
  if (the_pipeline)
    {
      ...
      newjob->state
          = any_running ? JRUNNING : (any_stopped ? JSTOPPED : JDEAD);
      newjob->wd = job_working_directory ();
      newjob->deferred = deferred;

      newjob->j_cleanup = nullptr;
      newjob->cleanarg = nullptr;
      ...
      if (newjob->state == JDEAD)
        {
          js.c_reaped += n; /* wouldn't have been done since this was not part
                               of a job */
          js.j_ndead++;
        }
      js.c_injobs += n;
...
/* The number of elements in bgpids.storage always has to be > js.c_childmax
   for the circular buffer to work right. */
void
Shell::bgp_resize ()
{
  ...
  if (bgpids.nalloc < nsize_cur && bgpids.nalloc < nsize_max)
    {
      bgpids.storage = (struct pidstat *)xrealloc (
          bgpids.storage, nsize * sizeof (struct pidstat));
...
Shell::bgp_getindex ()
{
  if (bgpids.nalloc < (ps_index_t)js.c_childmax
      || bgpids.head >= bgpids.nalloc)
    bgp_resize ();
...
ps_index_t *
Shell::pshash_getbucket (pid_t pid)
{
...
struct pidstat *
Shell::bgp_add (pid_t pid, int status)
{
  ...
  if (psi == *bucket)
    {
      internal_debug (
          "hashed pid %d (pid %d) collides with bgpids.head, skipping", psi,
          pid);
...
void
Shell::pshash_delindex (ps_index_t psi)
{
  struct pidstat *ps;
  ps_index_t *bucket;
...
Shell::bgp_delete (pid_t pid)
{
  ...
  for (orig_psi = psi = *(pshash_getbucket (pid)); psi != NO_PIDSTAT;
       psi = bgpids.storage[psi].bucket_next)
    {
      ...
      if (orig_psi == bgpids.storage[psi].bucket_next) /* catch reported bug */
        {
          internal_warning (
              _ ("bgp_delete: LOOP: psi (%d) == storage[psi].bucket_next"),
              psi);
          return 0;
        }
    }
  ...
  for (orig_psi = psi = *(pshash_getbucket (pid)); psi != NO_PIDSTAT;
       psi = bgpids.storage[psi].bucket_next)
    {
      ...
      if (orig_psi == bgpids.storage[psi].bucket_next) /* catch reported bug */
        {
          internal_warning (
              _ ("bgp_search: LOOP: psi (%d) == storage[psi].bucket_next"),
              psi);
          return -1;
        }
    }
...
void
Shell::save_proc_status (pid_t pid, int status)
{
  sigset_t set, oset;
      ...
      while (js.j_lastj != old)
        {
          ...
          if (jobs[js.j_lastj]
              || js.j_lastj == old) /* needed if old == js.j_jobslots */
            break;
          js.j_lastj--;
        }
      if (js.j_lastj == old)
  ...
  for (i = 0; i < js.j_jobslots; i++)
    {
      if (i < js.j_firstj && jobs[i])
        INTERNAL_DEBUG (
            ("cleanup_dead_jobs: job %d non-null before js.j_firstj (%d)", i,
             js.j_firstj));
      if (i > js.j_lastj && jobs[i])
        INTERNAL_DEBUG (
            ("cleanup_dead_jobs: job %d non-null after js.j_lastj (%d)", i,
             js.j_lastj));
...
delete_old_job (pid_t pid)
{
  ...
  if (job != NO_JOB)
    {
      INTERNAL_DEBUG (("delete_old_job: found pid %d in job %d with state %d",
                       pid, job, jobs[job]->state));
      if (JOBSTATE (job) == JDEAD)
        delete_job (job, DEL_NOBGPID);
      else
        {
          internal_debug (_ ("forked pid %d appears in running job %d"), pid,
                          job + 1);
          if (p)
            p->pid = 0;
        }
...
realloc_jobs_list ()
{
  ...
  BLOCK_CHILD (set, oset);
  nlist = (js.j_jobslots == nsize) ? jobs
                                   : (JOB **)xmalloc (nsize * sizeof (JOB *));
...
delete_job (int job_index, int dflags)
{
  ...
  if ((dflags & DEL_NOBGPID) == 0
      && (temp->flags & (J_ASYNC | J_FOREGROUND)) == J_ASYNC)
    {
  ...
  if (temp->state == JDEAD)
    {
      ...
      if (js.c_reaped < 0)
        {
          INTERNAL_DEBUG (("delete_job (%d pgrp %d): js.c_reaped (%d) < 0 "
                           "ndel = %d js.j_ndead = %d",
                           job_index, temp->pgrp, js.c_reaped, ndel,
                           js.j_ndead));
          js.c_reaped = 0;
        }
    }
...
add_process (char *name, pid_t pid)
{
  ...
  if (p)
    {
      if (j == NO_JOB)
        internal_debug ("add_process: process %5ld (%s) in the_pipeline",
                        (long)p->pid, p->command);
      if (PALIVE (p))
        internal_warning (
            _ ("add_process: pid %5ld (%s) marked as still alive"),
            (long)p->pid, p->command);
...
/* Map FUNC over the list of jobs.  If FUNC returns non-zero,
   ...
   and INDEX. */
int
Shell::map_over_jobs (sh_job_map_func_t *func, int arg1, int arg2)
{
  int i;
  int result;
  sigset_t set, oset;
  ...
  for (i = result = 0; i < js.j_jobslots; i++)
    {
      if (i < js.j_firstj && jobs[i])
        INTERNAL_DEBUG (
            ("map_over_jobs: job %d non-null before js.j_firstj (%d)", i,
             js.j_firstj));
      if (i > js.j_lastj && jobs[i])
        INTERNAL_DEBUG (
            ("map_over_jobs: job %d non-null after js.j_lastj (%d)", i,
             js.j_lastj));
...
find_pipeline (pid_t pid, int alive_only, int *jobp)
{
...
#if defined(PROCESS_SUBSTITUTION)
  if (procsubs.nproc > 0 && (p = procsub_search (pid))
      && ((alive_only == 0 && PRECYCLED (p) == 0) || PALIVE (p)))
    return p;
#endif
  ...
  if (jobp)
    *jobp = job;
  return (job == NO_JOB) ? nullptr : jobs[job]->pipe;
}
...
find_job (pid_t pid, int alive_only, PROCESS **procp)
{
  ...
  for (i = 0; i < js.j_jobslots; i++)
    {
      if (i < js.j_firstj && jobs[i])
        INTERNAL_DEBUG (("find_job: job %d non-null before js.j_firstj (%d)",
                         i, js.j_firstj));
      if (i > js.j_lastj && jobs[i])
        INTERNAL_DEBUG (("find_job: job %d non-null after js.j_lastj (%d)", i,
                         js.j_lastj));
      ...
      if (job_control)
        {
          ...
          if ((flags & FORK_NOTERM) == 0 && async_p == 0
              ...
              && ((subshell_environment & (SUBSHELL_ASYNC | SUBSHELL_PIPE))
                  == 0)
      ...
      if (tcsetattr (tty, TCSADRAIN, &shell_tty_info) < 0)
        {
          /* Only print an error message if we're really interactive at
             this time. */
          if (interactive)
            sys_error ("[%ld: %d (%d)] tcsetattr", (long)getpid (),
                       shell_level, tty);
          return -1;
        }
#endif /* TERMIOS_TTY_DRIVER */
...
/* Given an index into the jobs array JOB, return the PROCESS struct of the
   last process in that job's pipeline.  This is the one whose exit status
  ...
  for (njobs = any_stopped = 0, check_async = 1;;)
    {
      ...
      for (i = 0; i < js.j_jobslots; i++)
        {
          if (i < js.j_firstj && jobs[i])
            INTERNAL_DEBUG (("wait_for_background_pids: job %d non-null "
                             "before js.j_firstj (%d)",
                             i, js.j_firstj));
          if (i > js.j_lastj && jobs[i])
            INTERNAL_DEBUG (("wait_for_background_pids: job %d non-null after "
                             "js.j_lastj (%d)",
                             i, js.j_lastj));

          if (jobs[i] && STOPPED (i))
            {
              builtin_warning ("job %d[%d] stopped", i + 1,
                               find_last_pid (i, 0));
              any_stopped = 1;
            }
    ...
    {                                                                         \
      ...
      if (child == 0)                                                         \
        {                                                                     \
          ...
          UNBLOCK_CHILD (oset);                                               \
          internal_error (_ ("wait_for: No record of process %ld"),           \
                          (long)pid);                                         \
          restore_sigint_handler ();                                          \
  ...
  if (job_control == 0 || (subshell_environment & SUBSHELL_COMSUB))
    {
      ...
      if (temp_sigint_handler == wait_sigint_handler)
        internal_debug ("wait_for: recursively setting old_sigint_handler to "
                        "wait_sigint_handler: running_trap = %d",
                        running_trap);
      else
  ...
  if (job != NO_JOB)
    {
      if (interactive_shell && subshell_environment == 0)
          ...
          if (job_control && IS_JOBCONTROL (job) && IS_FOREGROUND (job)
              && WIFSIGNALED (s) && WTERMSIG (s) == SIGINT)
            {
              ...
              if (signal_is_trapped (SIGINT) == 0
                ...
                ADDINTERRUPT;
              /* Call any SIGINT trap handler if the shell is running a loop,
                 so the loop can be broken.  This seems more useful and matches
                 the behavior when the shell is running a builtin command in a
                 loop when it is interrupted.  Change ADDINTERRUPT to
              ...
              else if (interactive_shell && signal_is_trapped (SIGINT) == 0
                       && sourcelevel)
                ADDINTERRUPT;
              else
                {
      ...
      else if ((subshell_environment & (SUBSHELL_COMSUB | SUBSHELL_PIPE))
               ...
               && IS_FOREGROUND (job))
        {
          ...
          if (job_control && IS_JOBCONTROL (job) && WIFSIGNALED (s)
              && WTERMSIG (s) == SIGINT)
            {
      ...
      /* Moved here from set_job_status_and_cleanup, which is in the SIGCHLD
         signal handler path */
      if (DEADJOB (job)
          && IS_FOREGROUND (job) /*&& subshell_environment == 0*/)
        setjstatus (job);
...
set_current_job (int job)
{
  ...
  if (js.j_previous != js.j_current && js.j_previous != NO_JOB
      && jobs[js.j_previous] && STOPPED (js.j_previous))
    return;
...
reset_current ()
{
  ...
  if (js.j_jobslots && js.j_current != NO_JOB && jobs[js.j_current]
  ...
  else
    {
      ...
      if (js.j_previous != NO_JOB && jobs[js.j_previous]
...
start_job (int job, bool foreground)
{
  ...
  if (!foreground && already_running)
    {
      internal_error (_ ("%s: job %d already in background"),
                      this_command_name, job + 1);
      UNBLOCK_CHILD (oset);
  ...
  do
    {
      printf ("%s%s", p->command ? p->command : "",
      ...
      if (job != NO_JOB)
        {
          ...
          else if (jobs[job]->pgrp
                   == shell_pgrp) /* XXX - IS_JOBCONTROL(job) == 0? */
            {
              p = jobs[job]->pipe;
              do
                {
...
itrace("waitchld: waitpid returns %d block = %d children_exited = %d", pid, block, children_exited);
      ...
      if (wait_sigint_received
          && (WIFSIGNALED (status) == 0 || WTERMSIG (status) != SIGINT))
        child_caught_sigint = 1;
  ...
  if (JOBSTATE (job) == JDEAD)
    {
      ...
      if (wait_sigint_received && interactive_shell == 0 && child_caught_sigint
          && IS_FOREGROUND (job) && signal_is_trapped (SIGINT))
        {
          int old_frozen;
       ...
       */
      else if (wait_sigint_received && child_caught_sigint == 0
      ...
      if (jobs[job] && IS_NOTIFIED (job) == 0)
        {
          ...
          if (startup_state == 0 && WIFSIGNALED (s) == 0
              && ((DEADJOB (job) && IS_FOREGROUND (job) == 0)
                  || STOPPED (job)))
            continue;
          ...
          if ((job_control == 0 && interactive_shell)
              ...
              || (startup_state == 2 && posixly_correct
              ...
              if (tty_sigs++ > 16)
                {
                  ...
                  job_control = false;
                  original_pgrp
                      = terminal_pgrp; /* for eventual give_terminal_to */
                  goto just_bail;
                }
              continue;
            }
...
debug_print_pgrps ()
{
  ...
  itrace ("tcgetpgrp(%d) -> %ld, getpgid(0) -> %ld", shell_tty,
          (long)tcgetpgrp (shell_tty), (long)getpgid (0));
  ...
  if (tpgrp == npgrp)
  ...
  else if (tpgrp != opgrp)
    {
      internal_debug ("%d: maybe_give_terminal_to: terminal pgrp == %d shell "
                      "pgrp = %d new pgrp = %d in_background = %d",
                      (int)getpid (), tpgrp, opgrp, npgrp,
                      running_in_background);
      return -1;
    }
  else
      ...
      for (i = 0; i < js.j_jobslots; i++)
        {
          if (i < js.j_firstj && jobs[i])
            INTERNAL_DEBUG (
                ("delete_all_jobs: job %d non-null before js.j_firstj (%d)", i,
                 js.j_firstj));
          if (i > js.j_lastj && jobs[i])
            INTERNAL_DEBUG (
                ("delete_all_jobs: job %d non-null after js.j_lastj (%d)", i,
                 js.j_lastj));

          if (jobs[i] && (running_only == 0 || (running_only && RUNNING (i))))
            /* We don't want to add any of these pids to bgpids.  If
               running_only is non-zero, we don't want to add running jobs to
               the list. If we are interested in all jobs, not just running
               jobs, and we are going to clear the bgpids list below
               (bgp_clear()), we don't need to bother. */
  ...
  for (i = n = 0; i < js.j_jobslots; i++)
    {
      if (i < js.j_firstj && jobs[i])
        INTERNAL_DEBUG (
            ("count_all_jobs: job %d non-null before js.j_firstj (%d)", i,
             js.j_firstj));
      if (i > js.j_lastj && jobs[i])
        INTERNAL_DEBUG (
            ("count_all_jobs: job %d non-null after js.j_lastj (%d)", i,
             js.j_lastj));
...
mark_dead_jobs_as_notified (int force)
{
      ...
      for (i = 0; i < js.j_jobslots; i++)
        {
          if (jobs[i] && DEADJOB (i)
              && (interactive_shell
                  || (find_last_pid (i, 0) != last_asynchronous_pid)))
  ...
  for (i = ndead = ndeadproc = 0; i < js.j_jobslots; i++)
    {
      if (i < js.j_firstj && jobs[i])
        INTERNAL_DEBUG (("mark_dead_jobs_as_notified: job %d non-null before "
                         "js.j_firstj (%d)",
                         i, js.j_firstj));
      if (i > js.j_lastj && jobs[i])
        INTERNAL_DEBUG (("mark_dead_jobs_as_notified: job %d non-null after "
                         "js.j_lastj (%d)",
                         i, js.j_lastj));
  ...
  if (ndead != js.j_ndead)
    INTERNAL_DEBUG (
        ("mark_dead_jobs_as_notified: ndead (%d) != js.j_ndead (%d)", ndead,
         js.j_ndead));
  ...
  for (i = 0; i < js.j_jobslots; i++)
    {
      if (jobs[i] && DEADJOB (i)
        ...
        {
          if (i < js.j_firstj && jobs[i])
            INTERNAL_DEBUG (("mark_dead_jobs_as_notified: job %d non-null "
                             "before js.j_firstj (%d)",
                             i, js.j_firstj));
          if (i > js.j_lastj && jobs[i])
            INTERNAL_DEBUG (("mark_dead_jobs_as_notified: job %d non-null "
