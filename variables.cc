...
constexpr char *BASHFUNC_PREFIX = "BASH_FUNC_";
constexpr char *BASHFUNC_SUFFIX = "%%";

/* flags for find_variable_internal */

constexpr int FV_FORCETEMPENV = 0x01;
constexpr int FV_SKIPINVISIBLE = 0x02;
constexpr int FV_NODYNAMIC = 0x04;
...
static SHELL_VAR *hash_lookup (const char *, HashTable *);
static SHELL_VAR *new_shell_variable (const char *);
static SHELL_VAR *make_new_variable (const char *, HashTable *);
static SHELL_VAR *bind_variable_internal (const char *, const char *, HashTable *, int, int);
...
static void flatten (HashTable *, sh_var_map_func_t *, VarList *, int);

static int qsort_var_comp (const SHELL_VAR **, const SHELL_VAR **);
...
static SHELL_VAR *find_nameref_at_context (SHELL_VAR *, VarContext *);
static SHELL_VAR *find_variable_nameref_context (SHELL_VAR *, VarContext *, VarContext **);
static SHELL_VAR *find_variable_last_nameref_context (SHELL_VAR *, VarContext *, VarContext **);

static SHELL_VAR *bind_tempenv_variable (const char *, const char *);
static void push_posix_temp_var (void *);
...
static char **make_var_export_array (VarContext *);
...
static void push_builtin_var (void *);
...
Shell::init_variables (char **env, bool privmode)
{
      ...
      if (privmode == 0 && read_but_dont_execute == 0
          ...
          && STREQN ("() {", string, 4))
        {
          ...
          if (absolute_program (tname) == 0
              && (posixly_correct == 0 || legal_identifier (tname)))
  ...
  if (interactive_shell)
    {
      ...
      set_if_not ("PS1", (char *)primary_prompt);
#else
      if (current_user.uid == -1)
        get_current_user_info ();
      set_if_not ("PS1", current_user.euid == 0 ? "# " : primary_prompt);
#endif
      set_if_not ("PS2", (char *)secondary_prompt);
    }
  ...
  if (interactive_shell)
    {
      temp_var
          = set_if_not ("MAILCHECK", (char *)(posixly_correct ? "600" : "60"));
  ...
  if (command_execution_string)
    bind_variable ("BASH_EXECUTION_STRING", (char *)command_execution_string,
                   0);
...
set_machine_vars ()
{
  ...
  temp_var = set_if_not ("HOSTNAME", (char *)current_host_name);
}
...
adjust_shell_level (int change)
{
  ...
  if (old_SHLVL == 0 || *old_SHLVL == '\0'
      || legal_number (old_SHLVL, &old_level) == 0)
    old_level = 0;
...
set_pwd ()
{
  ...
  if (temp_var && imported_p (temp_var)
      && (temp_string = value_cell (temp_var)) && temp_string[0] == '/'
      && same_file (temp_string, ".", (struct stat *)NULL,
                    (struct stat *)NULL))
    {
      current_dir
          = sh_canonpath (temp_string, PATH_CHECKDOTDOT | PATH_CHECKEXISTS);
      if (current_dir == 0)
...
static SHELL_VAR *
null_assign (SHELL_VAR *self, const char *value, arrayind_t unused,
             const char *key)
{
  return self;
}
...
#if defined(ARRAY_VARS)
static SHELL_VAR *
null_array_assign (SHELL_VAR *self, const char *value, arrayind_t ind,
                   const char *key)
{
  return self;
}
...
static SHELL_VAR *
init_dynamic_array_var (const char *name, sh_var_value_func_t *getfunc,
...
static SHELL_VAR *
init_dynamic_assoc_var (const char *name, sh_var_value_func_t *getfunc,
                        sh_var_assign_func_t *setfunc, int attrs)
{
  SHELL_VAR *v;
...
static SHELL_VAR *
assign_seconds (SHELL_VAR *self, const char *value, arrayind_t unused,
                const char *key)
{
  int64_t nval;
  bool expok;
...
static SHELL_VAR *
assign_random (SHELL_VAR *self, const char *value, arrayind_t unused,
               const char *key)
{
  int64_t seedval;
  bool expok;
...
static SHELL_VAR *
assign_lineno (SHELL_VAR *var, const char *value, arrayind_t unused,
               const char *key)
{
  int64_t new_value;
...
static SHELL_VAR *
assign_subshell (SHELL_VAR *var, const char *value, arrayind_t unused,
                 const char *key)
{
  int64_t new_value;
...
get_epochrealtime (SHELL_VAR *var)
{
  ...
  snprintf (buf, sizeof (buf), "%u%c%06u", (unsigned)tv.tv_sec,
            locale_decpoint (), (unsigned)tv.tv_usec);
...
static SHELL_VAR *
assign_bash_argv0 (SHELL_VAR *var, const char *value, arrayind_t unused,
                   const char *key)
{
  size_t vlen;
  ...
  vlen = STRLEN (value);
  static_shell_name = (char *)xrealloc (static_shell_name, vlen + 1);
...
/* When this function returns, rl_completer_word_break_characters points to
   malloced memory. */
static SHELL_VAR *
assign_comp_wordbreaks (SHELL_VAR *self, const char *value, arrayind_t unused,
                        const char *key)
{
...
#if defined(PUSHD_AND_POPD) && defined(ARRAY_VARS)
static SHELL_VAR *
assign_dirstack (SHELL_VAR *self, const char *value, arrayind_t ind,
                 const char *key)
{
...
get_bashargcv (SHELL_VAR *self)
{
  ...
  if (self_semaphore == 0 && variable_context == 0
...
static SHELL_VAR *
assign_hashcmd (SHELL_VAR *self, const char *value, arrayind_t ind,
                const char *key)
{
...
static SHELL_VAR *
assign_aliasvar (SHELL_VAR *self, const char *value, arrayind_t ind,
                 const char *key)
{
...
init_funcname_var ()
{
  ...
  if (v)
    return v;
#if defined(ARRAY_VARS)
  INIT_DYNAMIC_ARRAY_VAR ((char *)"FUNCNAME", get_funcname, null_array_assign);
#else
...
initialize_dynamic_variables ()
{
  ...
  INIT_DYNAMIC_VAR ("BASH_ARGV0", (char *)NULL, get_bash_argv0,
                    assign_bash_argv0);
  ...
  INIT_DYNAMIC_VAR ("SRANDOM", (char *)NULL, get_urandom,
                    (sh_var_assign_func_t *)NULL);
  VSETATTR (v, att_integer);
  ...
  INIT_DYNAMIC_VAR ("EPOCHSECONDS", (char *)NULL, get_epochseconds,
                    null_assign);
  VSETATTR (v, att_regenerate);
...
#if defined(READLINE)
  INIT_DYNAMIC_VAR ("COMP_WORDBREAKS", (char *)NULL, get_comp_wordbreaks,
...
#if defined(DEBUGGER)
  v = init_dynamic_array_var ("BASH_ARGC", get_bashargcv, null_array_assign,
                              att_noassign | att_nounset);
  v = init_dynamic_array_var ("BASH_ARGV", get_bashargcv, null_array_assign,
                              att_noassign | att_nounset);
#endif /* DEBUGGER */
  ...
  v = init_dynamic_assoc_var ("BASH_CMDS", get_hashcmd, assign_hashcmd,
                              att_nofree);
#if defined(ALIAS)
  v = init_dynamic_assoc_var ("BASH_ALIASES", get_aliasvar, assign_aliasvar,
                              att_nofree);
#endif
#endif
...
static SHELL_VAR *
hash_lookup (const char *name, HashTable *hashed_vars)
{
...
SHELL_VAR *
var_lookup (const char *name, VarContext *vcontext)
{
  VarContext *vc;
  SHELL_VAR *v;
...
static SHELL_VAR *
find_nameref_at_context (SHELL_VAR *v, VarContext *vc)
{
  SHELL_VAR *nv, *nv2;
  char *newname;
  int level;
...
static SHELL_VAR *
find_variable_nameref_context (SHELL_VAR *v, VarContext *vc, VarContext **nvcp)
{
  SHELL_VAR *nv, *nv2;
  VarContext *nvc;
...
/* Do nameref resolution from the VC, which is the local context for some
   ...
   variable context and hash table). */
static SHELL_VAR *
find_variable_last_nameref_context (SHELL_VAR *v, VarContext *vc,
                                    VarContext **nvcp)
{
  SHELL_VAR *nv, *nv2;
  VarContext *nvc;
...
{
#if defined(DEBUGGER)
  return (Function *)hash_lookup (name, shell_function_defs);
#else
  return (Function *)0;
#endif
}
...
make_local_variable (const char *name, int flags)
{
  ...
  /* If we're making a local variable in a shell function, the temporary env
     ...
     VarContext and can safely be returned without creating a new variable
     (which results in duplicate names in the same VarContext->table */
  /* We can't just test tmpvar_p because variables in the temporary env given
     to a shell function appear in the function's local variable VarContext
...
/* Create a new shell variable with name NAME and add it to the hash table
   TABLE. */
SHELL_VAR *
Shell::make_new_variable (const char *name, HashTable *table)
{
  SHELL_VAR *entry;
  BucketContents *elt;
...
#if defined(ARRAY_VARS)
SHELL_VAR *
Shell::make_new_array_variable (const char *name)
{
  SHELL_VAR *entry;
  ARRAY *array;
...
SHELL_VAR *
Shell::make_local_array_variable (const char *name, int flags)
{
  SHELL_VAR *var;
  ARRAY *array;
  int assoc_ok;
  ...
  if (localvar_inherit && assoc_p (var))
    {
      internal_warning ("%s: cannot inherit value from incompatible type",
                        name);
  ...
  if ((flags & ASS_NOEVAL) == 0 && integer_p (var))
    {
      ...
      if (flags & ASS_APPEND)
        {
          const char *oval = value_cell (var);
      ...
      retval = itos (rval);
           ...
           && (capcase_p (var) || uppercase_p (var) || lowercase_p (var)))
    {
      if (flags & ASS_APPEND)
        {
      ...
      char *newval = sh_modcase (retval, (char *)0, op);
      free (retval);
      retval = newval;
    }
#endif /* CASEMOD_ATTRS */
...
can_optimize_assignment (SHELL_VAR *entry, const char *value, int aflags)
{
  ...
  if (array_p (entry) || assoc_p (entry))
    return false;
#endif
  if (integer_p (entry) || uppercase_p (entry) || lowercase_p (entry)
      || capcase_p (entry))
    return false;
...
optimized_assignment (SHELL_VAR *entry, const char *value, int aflags)
{
  ...
  new_ = (char *)xrealloc (v, len + vlen + 8); /* for now */
  if (vlen == 1)
    {
      new_[len] = *value;
      new_[len + 1] = '\0';
...
/* Bind a variable NAME to VALUE in the HashTable TABLE, which may be the
...
static SHELL_VAR *
bind_variable_internal (const char *name, const char *value, HashTable *table,
                        int hflags, int aflags)
{
  ...
  entry
      = (hflags & HASH_NOSRCH) ? (SHELL_VAR *)NULL : hash_lookup (name, table);
...
bind_int_variable (const char *lhs, const char *rhs, int flags)
{
  ...
  if (valid_array_reference (lhs, (flags & ASS_NOEXPAND) != 0))
    {
      isarr = 1;
      v = array_variable_part (lhs, (flags & ASS_NOEXPAND) != 0, (char **)0,
                               (int *)0);
    }
...
void
bind_function_def (const char *name, Function *value, int flags)
{
  Function *entry;
  BucketContents *elt;
  COMMAND *cmd;
...
/* Add STRING, which is of the form foo=bar, to the temporary environment
   HashTable (temporary_env).  The functions in execute_cmd.c are
...
int
delete_var (const char *name, VarContext *vc)
{
  BucketContents *elt;
  SHELL_VAR *old_var;
  VarContext *v;

  for (elt = (BucketContents *)NULL, v = vc; v; v = v->down)
...
int
makunbound (const char *name, VarContext *vc)
{
  BucketContents *elt, *new_elt;
  SHELL_VAR *old_var;
  VarContext *v;
  char *t;
...
kill_all_local_variables ()
{
  ...
  vc->table = (HashTable *)NULL;
}

static void
free_variable_hash_data (void *data)
{
  SHELL_VAR *var;
...
void
delete_all_variables (HashTable *hashed_vars)
{
...
SHELL_VAR **
map_over (sh_var_map_func_t *function, VarContext *vc)
{
  VarContext *v;
  VarList *vlist;
  SHELL_VAR **ret;
...
/* Flatten VarList, applying FUNC to each member and adding those
   ...
   NULL, each variable in VarList is added to VLIST.  If VLIST is
   NULL, FUNC is applied to each SHELL_VAR in VarList.  If VLIST
   and FUNC are both NULL, nothing happens. */
static void
flatten (HashTable *var_hash_table, sh_var_map_func_t *func, VarList *vlist,
         int flags)
{
  int i;
  BucketContents *tlist;
  int r;
  SHELL_VAR *var;
...
/* Take all of the shell variables in the temporary environment HashTable
...
static inline char *
mk_env_string (const char *name, const char *value, int isfunc)
{
...
static char **
make_var_export_array (VarContext *vcxt)
{
  char **list;
  SHELL_VAR **vars;
...
new_var_context (const char *name, int flags)
{
  ...
  vc = (VarContext *)xmalloc (sizeof (VarContext));
  vc->name = name ? savestring (name) : (char *)NULL;
  vc->scope = variable_context;
  vc->flags = flags;

  vc->up = vc->down = (VarContext *)NULL;
  vc->table = (HashTable *)NULL;
...
void
dispose_var_context (VarContext *vc)
{
  FREE (vc->name);
...
/* Make a new variable context with NAME and FLAGS and a HashTable of
   temporary variables, and push it onto shell_variables.  This is
   for shell functions. */
VarContext *
push_var_context (const char *name, int flags, HashTable *tempvars)
{
  VarContext *vc;
...
/* Delete the HashTables for all variable contexts beginning at VCXT, and
   all of the VarContexts except GLOBAL_VARIABLES. */
void
delete_all_contexts (VarContext *vcxt)
{
  VarContext *v, *t;
...
VarContext *
push_scope (int flags, HashTable *tmpvars)
{
...
void
push_context (const char *name, bool is_subshell, HashTable *tempvars)
{
  if (!is_subshell)
    push_dollar_vars ();
  variable_context++;
...
set_pipestatus_array (int *ps, int nproc)
{
  ...
  if (v == 0)
    v = make_new_array_variable (
        (char *)"PIPESTATUS"); /* XXX can this be freed? */
  if (array_p (v) == 0)
