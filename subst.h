...
extern char *sub_append_string (char *, char *, int *, size_t *);
...
extern char *expand_string_to_string (const char *, int);
extern char *expand_string_unsplit_to_string (const char *, int);
extern char *expand_assignment_string_to_string (const char *, int);

/* Expand an arithmetic expression string */
extern char *expand_arith_string (const char *, int);
...
extern const char *getifs ();

/* This splits a single word into a WORD LIST on $IFS, but only if the word
   is not quoted.  list_string () performs quote removal for us, even if we
   don't do any splitting. */
extern WORD_LIST *word_split (WORD_DESC *, const char *);
...
extern bool assigning_in_environment;
extern bool expanding_redir;
...
extern void invalidate_cached_quoted_dollar_at ();
